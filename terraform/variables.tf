...
variable "image_name" {
  description = <<-EOT
    OS イメージ名。`openstack image list` で正確な名称を確認すること。
    例: vmi-debian-12.0-amd64 / vmi-ubuntu-24.04-amd64
    cloud-init 対応の Debian/Ubuntu 系を推奨。
  EOT
  type        = string
  default     = "vmi-debian-12.0-amd64"
}
