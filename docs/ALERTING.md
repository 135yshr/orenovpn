...
| 不審な出口通信 | 既知悪性 IP への通信を FORWARD ログで検知（ログのみ・ドロップしない） | 中 |
...
1. 軽量ログ監視 + メール通知（SSH 失敗・新規接続・トラフィック量）— 本実装で有効。
2. 出口通信検知（`alert_blocklist_url` 設定 + サーバー側の FORWARD ログルール）— ブロック
   リストの取り込みとファイアウォール連携が必要なため、ufw 一本化の原則に沿った追加実装を
   後続で行う。`watch.sh` 側の検知ロジックは実装済みで、ログ出力を有効化すれば作動する。
