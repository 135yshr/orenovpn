...
|-----------|---------|-----------|-----------|------|
| `simple` | WireGuard | 全開放 | 22 | まず試す / 接続元IPが変わる環境 |
| `balanced` | WireGuard | 自分のIPに制限 | 22 | 常用の推奨ベースライン |
| `hardened` | WireGuard | 固定IPのみ厳格 | 22 | セキュリティ最優先 |
| `ikev2` | IKEv2 | 全開放 | 22 | iPhone/macOS 標準VPNでアプリ不要接続 |

> **SSH ポートについて**: 既定は 22 です。22 以外に変更すると Debian 13 等の
> SSH ソケットアクティベーションで反映されず接続不能になる場合があるため、
> 変更は非推奨（上級者向け）。防御は鍵認証＋fail2ban（＋接続元IP制限）が担うため
> 22 のままでも安全です。
...
```bash
# サーバーから設定ファイルを取得
scp -P 22 vpnadmin@<サーバーIP>:/etc/orenovpn/clients/client1.conf ./
